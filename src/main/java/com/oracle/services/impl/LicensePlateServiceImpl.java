...
public class LicensePlateServiceImpl implements LicensePlateService {
  ...
  public List<Plate> findPlate(String requestedPlateNumber) {
    ...
    try {
      Query query = em.createQuery("Select a FROM Plate a where a.plateNumber like :requestedPlateNumber order by a.ts desc").setParameter("requestedPlateNumber", requestedPlateNumber);
